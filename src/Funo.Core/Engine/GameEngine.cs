...
public static class GameEngine
{
    ...
    {
        if (state.IsFinished)
            return PlayResult.Fail(EngineMessages.GameAlreadyFinished);

        var player = state.CurrentPlayer;

        if (!player.HasCard(card))
            return PlayResult.Fail(EngineMessages.CardNotInHand);

        if (!CanPlay(state, card))
            return PlayResult.Fail(EngineMessages.CardNotPlayable);

        if (card.IsWild)
        {
            if (chosenColor is null or CardColor.Wild)
                return PlayResult.Fail(EngineMessages.ColorRequired);
        }
        ...
        if (player.CardCount == 0)
        {
            ...
            state.Finish(player);
            return PlayResult.Ok(EngineMessages.PlayerFinished);
        }

        ApplyCardEffect(state, card);
        return PlayResult.Ok(EngineMessages.CardPlayed);
    }
    ...
    {
        if (state.IsFinished)
            return new DrawResult([], false, EngineMessages.GameAlreadyFinished);
        ...
        if (state.PendingDrawCount > 0)
        {
            ...
            return new DrawResult(penalty, false, EngineMessages.PenaltyDrawn);
        }
        ...
        return new DrawResult([card], playable, EngineMessages.CardDrawn);
    }
